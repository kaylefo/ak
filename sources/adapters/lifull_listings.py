"""LIFULL listing index and detail adapter."""
...
class LifullListingsAdapter:
    ...
    def _detect_page_kind(self, tree, url: str) -> str:
        if "/detail/" in url or tree.css_first(".property-detail, .bukken-detail"):
            return "detail"
        return "index"
    ...
    def _parse_index(self, tree, base_url: str) -> tuple[list[DiscoveredLink], list[ParsedListing]]:
        ...
        for selector in self._INDEX_SELECTORS:
            for card in tree.css(selector):
                ...
                title_node = card.css_first(".property-title, .bukken-card__title, h3")
                title = title_node.text(strip=True) if title_node else None
                price_node = card.css_first(".price, .bukken-card__price")
                area_node = card.css_first(".area, .bukken-card__area")
                status_node = card.css_first(".status, .bukken-card__status")
                ...
                fields = {
                    ...
                    "募集状況": status_node.text(strip=True) if status_node else "",
                }
                listings.append(
                    listing_from_fields(
    ...
    def _parse_detail(self, tree, url: str) -> ParsedListing | None:
        fields = parse_dl_fields(tree, "dl.property-detail__list, dl.detail-list")
        if not fields:
            fields = parse_key_value_table(tree, "table.property-spec, table.detail-table")

        title_node = tree.css_first("h1.property-title, h1.bukken-detail__title")
        title = title_node.text(strip=True) if title_node else None
